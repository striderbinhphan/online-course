...
module.exports = {
  deleteUser(user_id) {
    return db("onlinecourse.user").where("user_id", user_id).del();
  },
  addLecture(lecture) {
    return db("onlinecourse.user").insert(lecture);
  },
  allLearner(page) {
    return  db.select('user_id', "user_username",'user_name','user_email','user_dob')
    .from('onlinecourse.user')
  ...
  allDisableLearner(page) {
    ...
    all(){
        return db('onlinecourse.user');
    },
    addNewUser(user){
        return db('onlinecourse.user').insert(user);
    },
    disableUser(userId) {
        return db("onlinecourse.user").where("user_id", userId).update(
            { 
                user_isdisable: 1,
      ...
      unDisableUser(userId) {
        return db("onlinecourse.user").where("user_id", userId).update({ user_isdisable: 0,user_status:"active" });
      },
    //kiem tra co ton tai user bang user name, tra ve user
    async isExistByUserId(userId){
        const ret = await db('onlinecourse.user').where('user_id',userId);
    ...
    async isExistByUsername(username){
        const ret = await db('onlinecourse.user').where('user_username',username);
    ...
    async isExistByEmail(email){
        const ret = await db('onlinecourse.user').where('user_email',email);
    ...
    addRFTokenToDB(user_id,refreshToken){
        return db('onlinecourse.user').where('user_id',user_id).update({
            refresh_token: refreshToken
        })
    },
     //kiem tra refresh token
    async isValidRFToken(user_id, refreshToken){
        const user = await db('onlinecourse.user').where('user_id',user_id);
        if(user.length === 0){
            return false;
        }
    ...
    addOTPTokenToDB(user_email,otpToken){
        return db('onlinecourse.user').where('user_email',user_email).update({
    ...
    updateUserInfo(user_username,user){
        return db('onlinecourse.user').where('user_username',user_username).update(user)
    },
    //update user email
    ...
    updatePassword(user_username, newPassword){
        return db('onlinecourse.user').where('user_username',user_username).update({
