...
module.exports = {
  all() {
    return db("onlinecourse.course").where("course_isdisable", 0);
  },
  getCourseInWeek() {
    const dt = new Date();
    dt.setDate(dt.getDate() - 7);

    return db("onlinecourse.course")
      .where("course_isdisable", 0)
      .where("created_date", "<=", new Date())
      .where("created_date", ">=", dt);
  },
  async getCourseById(course_id) {
    const cList = await db("onlinecourse.course")
  ...
  addNewCourse(course) {
    return db("onlinecourse.course").insert(course);
  },
  uploadCourseImage(course_id, course_image) {
    return db("onlinecourse.course").where("course_id", course_id).update({
      course_image: course_image,
    });
  },
  delete(courseId) {
    return db("onlinecourse.course").where("course_id", courseId).del();
  },
  ...
  updateCourse(courseId, course) {
    return db("onlinecourse.course").where("course_id", courseId).update(course);
  },
  ...
  allDisableCoursesForGuest(page) {
    return db("onlinecourse.course")
      ...
      .innerJoin("onlinecourse.user", "user.user_id", "course.user_id")
      .innerJoin("onlinecourse.category", "category.category_id", "course.category_id");
    // .limit(limit_of_page)
    // .offset((page - 1) * limit_of_page);
  },
  ...
  webCourses() {
    return db("onlinecourse.course").where("category_id", 1).where("course_isdisable", 0);
  },
  mobileCourses() {
    return db("onlinecourse.course").where("category_id", 2).where("course_isdisable", 0);
  },
  // getCourseById(id) {
  ...
  deleteCourse(course_id) {
    return db("onlinecourse.course").where("course_id", course_id).del();
  },
  getFeedBack(course_id) {
    return db
      .select("review_id", "review_feedback", "user_id", "timestamp")
      .from("onlinecourse.review")
      .where("course_id", course_id);
  },
  ...
  countSubcribeByCourseID(course_id) {
    return db("onlinecourse.course_subscribe")
  ...
  async getCoursesByCategoryId(categoryId) {
    const courses = await db("onlinecourse.course")
  ...
  async addViewEvent(courseId) {
    let today = new Date();
    const cViewEvent = await db("onlinecourse.user_view_event").where("course_id", courseId);
    if (cViewEvent.length === 0) {
      console.log("Dont existing in our db table");
      return db("onlinecourse.user_view_event").insert({
        course_id: courseId,
        viewcount: 1,
        date: today,
      });
      ...
      if (getFormattedDate(cViewEvent[0].date) !== getFormattedDate(today)) {
        ...
        return db("onlinecourse.user_view_event")
